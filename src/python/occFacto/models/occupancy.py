...
class DecompositionNetwork(nn.Module):
    ...
    def __init__(self, opt: Options_Occ, act=nnf.relu, norm_layer: nn.Module = nn.LayerNorm):
        ...
        if opt.decomposition_network == 'mlp':
            ...
            self.to_zb = MLP((opt.dim_h, *([2 * opt.dim_h] * opt.decomposition_num_layers), opt.dim_h))
        else:
            self.to_zb = Transformer(opt.dim_h, opt.num_heads, opt.num_layers, act=act,
...
class OccupancyMlP(nn.Module):
    ...
    def __init__(self, opt: Options_Occ):
        super(OccupancyMlP, self).__init__()
        dim_in = 2 * (opt.pos_dim + DIM)
...
class OccupancyNetwork(nn.Module):
    ...
    def __init__(self, opt: Options_Occ):
        ...
        self.pos_encoder = SineLayer(DIM, opt.pos_dim, is_first=True)

        if hasattr(opt, 'head_occ_type') and opt.head_occ_type == 'skip':
            self.occ_mlp = OccupancyMlP(opt)
        else:
            self.occ_mlp = MLP([(opt.pos_dim + DIM)] + [opt.dim_h] * opt.head_occ_size + [1])
        self.occ_transformer = Transformer(opt.pos_dim + DIM,
...
class DecompositionControl(Model):
    ...
    def __init__(self, opt: Options_Occ):
        ...
        if sum(opt.symmetric) > 0:
            ...
            self.reflect = None
        self.split_shape = tuple((DIM + 2) * [DIM] + [1])
...
class Spaghetti(Model):
    ...
    def __init__(self, opt: Options_Occ):
        ...
        if opt.use_encoder:
            self.mixing_network = Transformer(opt.dim_h, opt.num_heads, opt.num_layers,
                                                              act=nnf.relu, norm_layer=nn.LayerNorm)
        else:
            self.mixing_network = DummyTransformer()
        self.dist = None